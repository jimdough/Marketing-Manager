...
<?PHP
	echo "<h1>RSVP ADMIN Reminder Email Output</h1>";
	...
	fwrite($fh, "<h1>Logging RSVP ADMIN Reminders for " . $fullDate . "</h1>");
	...
	while($eventInfo = mysql_fetch_array($event))	{
		...
		if (($offset==10) OR ($offset==2)) {
		    ...
		    $recipientName = ucfirst($eventInfo['yourName']);
		    //
			
			$messageContent = "Dear " . $eventInfo['yourName'] . ",<br><br>";
			$messageContent .= "We wanted to remind you that the " . $eventInfo['eventName'] . " that you have scheduled for " . $eventInfo['eventDate1DayName'] . ", " .  $eventInfo['eventDate1Month'] . "/" . $eventInfo['eventDate1Day'] . "/" . $eventInfo['eventDate1Year'] . " is just a few days away. Please see the list of RSVPs below :<br>";
			...
			$messageContent .= "<li><b>" . ucfirst($rsvpInfo['firstName']) . " " .  ucfirst($rsvpInfo['lastName']) . "</b> confirmed attendance on "  . $rsvpInfo['date'] . ". " . ucfirst($rsvpInfo['firstName']) . "'s email is <a href='mailto:" .  $rsvpInfo['email'] . "'>" . $rsvpInfo['email'] . "</a> and their phone number is " . $rsvpInfo['phone'] . "</li>";
			 ...
			 $messageContent .= "</ul>";
			  ...
			  $message->setSubject("The " . $eventInfo['eventName'] . " is coming up on " . $eventInfo['eventDate1DayName'] . ", " .  $eventInfo['eventDate1Month'] . "/" . $eventInfo['eventDate1Day'] . "/" . $eventInfo['eventDate1Year']);
			
			  //Set the From address with an associative array
			  $message->setFrom('noreply@roadmaster.com');
			  
			  $message->setTo($eventInfo['rsvpEmail']);
			  
			  $message->setCc($eventInfo['yourEMail']);
