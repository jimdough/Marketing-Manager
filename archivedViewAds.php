...
<?PHP include("inc/topNav.php"); ?>
	...
	<form action="functions/adFunctions.php?function=unarchiveSelected" method="post" enctype="multipart/form-data">

<?PHP

		while($loopInfo = mysql_fetch_array($schoolLoop))
				  {
						$ads = mysql_query("SELECT * FROM admanager WHERE active=0 AND school=" . $loopInfo['id']);
						$header = mysql_query("SELECT * FROM admanager WHERE active=0 AND school=" . $loopInfo['id']);
			...
			while($adsInfo = mysql_fetch_array($ads))
				  ...
				  echo "<div class='button'><a href='functions/adfunctions.php?function=unarchiveAd&id=" . $adsInfo['id'] . "'>Restore this Ad</a></div>";
				  echo "</div>";
				  echo "<div class='clear'></div>";
				  }
}
?>
<div style="text-align:center;"><button class="button" type="submit" name="submit" id="submit">Restore Selected</button></div>
</form>
